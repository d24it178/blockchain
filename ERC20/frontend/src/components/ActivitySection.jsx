...
export default function ActivitySection() {
  ...
  const getStatusColor = (status) => {
    return status === 'completed' ? 'bg-emerald-500/20' : 'bg-orange-500/20'
  }
  ...
  return (
    <motion.div
      ...
      initial="hidden"
      animate="visible"
      className="glass-lg rounded-2xl overflow-hidden border border-indigo-500/30 shadow-xl shadow-indigo-500/10"
    >
      {/* Header */}
      <div className="px-6 py-4 border-b border-indigo-500/30 flex items-center gap-2 bg-gradient-to-r from-indigo-900/20 to-transparent">
        <Clock size={18} className="text-indigo-400" />
        <h3 className="font-bold text-lg text-slate-100">Transaction History</h3>
      </div>

      {/* Transaction List */}
      <div className="divide-y divide-indigo-500/20">
        {loading ? (
          <div className="px-6 py-8 flex items-center justify-center gap-2 text-slate-400 font-medium">
          ...
          transactions.slice(0, 10).map((tx, idx) => (
            <motion.div
              ...
              className="px-6 py-4 transition-colors cursor-pointer group border-l-4 border-l-emerald-500/0 hover:border-l-emerald-500/40"
            >
              <div className="flex items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-bold text-sm capitalize text-slate-200">
                      {tx.type === 'approve'
                        ? 'Authorization'
                        : `${tx.type === 'send' ? 'Outgoing' : 'Incoming'} Transfer`}
                    </div>
                    <div className="text-xs text-slate-400 truncate" title={tx.address}>
                ...
                <div className="flex flex-col items-end gap-1">
                  <div className={`font-bold text-sm ${getTypeColor(tx.type)}`}>
                    {tx.type === 'receive' ? '+' : '-'}
                    {formatNumber(parseFloat(tx.amount), 2)} {TOKEN_METADATA.symbol}
                  </div>
                  <motion.div
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    className={`text-xs px-2 py-1 rounded-full ${
                      tx.status === 'completed'
                        ...
                        : 'bg-orange-500/20 text-orange-300 animate-pulse font-semibold'
                    }`}
                  >
      ...
      {transactions.length > 0 && (
        <motion.button
          ...
          className="w-full px-6 py-3 text-sm font-bold text-emerald-400 hover:text-emerald-300 transition-colors border-t border-indigo-500/30 hover:bg-emerald-500/5"
        >
